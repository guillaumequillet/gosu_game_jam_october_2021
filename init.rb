...
$sounds = {
  'pallet_ok': Gosu::Sample.new('./sfx/423930__mudkip2016__correct.wav'),
  'box_ok': Gosu::Sample.new('./sfx/476178__unadamlar__correct-choice.wav'),
  'box_nok': Gosu::Sample.new('./sfx/478191__jonnyruss01__beep-error-1.ogg'),
  'box_missed': Gosu::Sample.new('./sfx/521973__kastenfrosch__error.ogg'),
  'box_placed': Gosu::Sample.new('./sfx/433770__zivs__ammo-pickup-1.ogg'),
  'alert': Gosu::Sample.new('./sfx/529628__beetlemuse__alert-2.wav')
}
...
class Conveyor
  SPEEDS = [0, 0.5, 1, 4]
  CONTROL_PANEL = [
    Gosu::Image.new('./gfx/conveyor_control_panel/stop.png', retro: true),
    Gosu::Image.new('./gfx/conveyor_control_panel/speed_1.png', retro: true),
    Gosu::Image.new('./gfx/conveyor_control_panel/speed_2.png', retro: true),
    Gosu::Image.new('./gfx/conveyor_control_panel/speed_3.png', retro: true)
  ]
  ...
  def initialize(window, x, y, w, h)
    ...
    @ejection_spots = []
    @boxes = []
    @stripe = Gosu::Image.new('./gfx/conveyor_stripe.png', retro: true)
    @offset_stripe = 0
    @speed = SPEEDS.first
  end
...
class Box
  ...
  WIDTH = 48
  HEIGHT = 32
  @@gfx = Gosu::Image.load_tiles('./gfx/box.png', WIDTH, HEIGHT, retro: true)
  VELOCITY = 10.0
...
class Game
  def initialize(window)
    ...
    @gfx = {
      ...
      tutorial: Gosu::Image.new('./gfx/tutorial.png', retro: true)
    }

    setup_game

    @font = Gosu::Font.new(48)
    @music = Gosu::Song.new('./sfx/funbgm102614.ogg')
  end
  ...
  def draw
    ...
    when :game
      ...
      @spawn_area.draw
      @conveyor.draw
      @font.draw_text("Score : #{$main_score}", 460, 500, 1, 1, 1, Gosu::Color::BLACK)
    when :game_over
      @gfx[:background].draw(0, 0, 0)
      @spawn_area.draw
      @conveyor.draw
